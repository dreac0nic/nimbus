...
public:
	...
	TEST_METHOD(initialTest)
	{
		...
		Assert::IsTrue(Util::closeEnough(rect.left, 1.3, 0.01));
		Assert::IsTrue(Util::closeEnough(rect.top, 4.0, 0.01));
		Assert::IsTrue(Util::closeEnough(rect.right, 3.4, 0.01));
		Assert::IsTrue(Util::closeEnough(rect.bottom, 7.0, 0.01));
		Assert::IsTrue(Util::closeEnough(rect.width, 2.1, 0.01));
		Assert::IsTrue(Util::closeEnough(rect.height, 3.0, 0.01));
	}
...
TEST_CLASS(LineSegmentTest){
	...
	TEST_METHOD(compareTest){
		...
		Assert::IsTrue(Util::closeEnough(LineSegment::compareLengths(&line1, &line2), -1, 0.1));
		Assert::IsTrue(Util::closeEnough(LineSegment::compareLengths(&line2, &line1), 1, 0.1));
		Assert::IsTrue(Util::closeEnough(LineSegment::compareLengths(&line1, &line1), 0, 0.1));

		
		Assert::IsTrue(Util::closeEnough(LineSegment::compareLengthsMax(&line1, &line2), 1, 0.1));
		Assert::IsTrue(Util::closeEnough(LineSegment::compareLengthsMax(&line2, &line1), -1, 0.1));
		Assert::IsTrue(Util::closeEnough(LineSegment::compareLengthsMax(&line1, &line1), 0, 0.1));
	}
};